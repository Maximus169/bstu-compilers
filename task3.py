...
with open('task.md') as data:
    data = data.read()
    data = re.sub('\n', '<br>', data)

    # заголовки
    data = re.sub(r'###### (.+?)<br>', r'<h6>\1</h6> <br>', data)
    data = re.sub(r'##### (.+?)<br>', r'<h5>\1</h5> <br>', data)
    data = re.sub(r'#### (.+?)<br>', r'<h4>\1</h4> <br>', data)
    data = re.sub(r'### (.+?)<br>', r'<h3>\1</h3> <br>', data)
